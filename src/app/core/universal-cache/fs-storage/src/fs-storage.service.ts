...
import { mkdirSync, readdirSync, readFileSync, rmdirSync, statSync, unlinkSync, writeFileSync, existsSync } from 'fs';
...
export class FsStorageService extends Storage {
  ...
  constructor(public loader: FsStorageLoader) {
    super();
  ...
  setItem(key: string, value: any): boolean {
    ...
    let oldValue = hasListeners ? this.getItem(key) : undefined;

    let item = this.metadata[key];
    let oldLength = !!item ? item.size : 0;
